...
import (
	...
	mmetrics "github.com/mansoormajeed/go-http-metrics/internal/mocks/metrics"
	"github.com/mansoormajeed/go-http-metrics/metrics"
...
func TestMiddleware(t *testing.T) {
	tests := map[string]struct {
		...
		"A default HTTP middleware using JSON should call the recorder to measure (Regression test: https://github.com/mansoormajeed/go-http-metrics/issues/31).": {
			req: func() *http.Request {
				return httptest.NewRequest(http.MethodPost, "/test", nil)
			},
			mock: func(m *mmetrics.Recorder) {
